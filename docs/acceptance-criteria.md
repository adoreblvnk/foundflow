...
- [x] Login-disabled mode blocks live provider scans at the shared provider boundary unless an explicit synthetic-demo exception is configured.
- [x] A configured staff identity can sign in through an HTTP-only signed session cookie.
- [x] Production fails closed without a valid data-encryption keyring; sensitive fields and item photos use authenticated AES-256-GCM envelopes, with database fields bound to stable row identity.
...
- [x] Security headers use request-specific script nonces and block un-nonced inline handlers, framing, MIME sniffing and unnecessary browser capabilities.
...
- [x] Staff can archive and restore any case without deleting its audit history or photos.
